...
for cc in reversed(ALL_COMPUTE_CAPABILITIES):
	...
	try:
		_C = importlib.import_module(f"tinycudann32_bindings._{cc}_C")
		if cc != system_compute_capability:
			warnings.warn(f"tinycudann32 was built for lower compute capability ({cc}) than the system's ({system_compute_capability}). Performance may be suboptimal.")
		break
	except ModuleNotFoundError:
		pass

if _C is None:
	raise EnvironmentError(f"Could not find compatible tinycudann32 extension for compute capability {system_compute_capability}.")

# Pipe tcnn warnings and errors into Python
# def _log(severity, msg):
# 	if severity == _C.LogSeverity.Warning:
# 		warnings.warn(f"tinycudann32 warning: {msg}")
# 	elif severity == _C.LogSeverity.Error:
# 		warnings.warn(f"tinycudann32 error: {msg}")
