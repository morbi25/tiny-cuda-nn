...
for cc in reversed(ALL_COMPUTE_CAPABILITIES):
	...
	try:
		_C = importlib.import_module(f"tinycudann16_bindings._{cc}_C")
		if cc != system_compute_capability:
			warnings.warn(f"tinycudann16 was built for lower compute capability ({cc}) than the system's ({system_compute_capability}). Performance may be suboptimal.")
		break
	except ModuleNotFoundError:
		pass

if _C is None:
	raise EnvironmentError(f"Could not find compatible tinycudann16 extension for compute capability {system_compute_capability}.")

# Pipe tcnn warnings and errors into Python
# def _log(severity, msg):
# 	if severity == _C.LogSeverity.Warning:
# 		warnings.warn(f"tinycudann16 warning: {msg}")
# 	elif severity == _C.LogSeverity.Error:
# 		warnings.warn(f"tinycudann16 error: {msg}")
